  ...
  <section class="ticket-detail">
    <header class="ticket-detail__header">
      ...
      <div class="ticket-detail__actions">
        ...
        <button class="button button--primary" :disabled="classifying" @click="runClassification">
    ...
    <div v-else-if="ticket" class="ticket-detail__content">
      ...
      <div class="ticket-detail__grid">
        ...
        <div class="ticket-detail__section">
          ...
          <div class="ticket-detail__readonly">{{ confidenceText }}</div>
        </div>
        ...
        <div class="ticket-detail__section ticket-detail__section--wide">
          ...
          <div class="ticket-detail__readonly ticket-detail__explanation" :class="{ 'ticket-detail__readonly--empty': !ticket.classification_explanation }">
            {{ ticket.classification_explanation || '—' }}
          </div>
        </div>
      </div>
...
<style>
.ticket-detail { max-width: 900px; margin: 0 auto; padding: 16px; }
.ticket-detail__header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; }
.ticket-detail__title { margin: 0; }
.ticket-detail__actions { display: flex; gap: 8px; }
.ticket-detail__loading, .ticket-detail__error { padding: 12px; color: #555; }
.ticket-detail__content { display: block; }
.ticket-detail__section { margin-bottom: 12px; }
.ticket-detail__section--wide { grid-column: 1 / -1; }
.ticket-detail__label { display: block; font-weight: 600; margin-bottom: 6px; }
.ticket-detail__subject { background: #fafafa; border: 1px solid #eee; padding: 8px; border-radius: 4px; }
.ticket-detail__body { background: #fff; border: 1px solid #eee; padding: 8px; border-radius: 4px; white-space: pre-wrap; }
.ticket-detail__grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
.ticket-detail__readonly { background: #fafafa; border: 1px solid #eee; padding: 8px; border-radius: 4px; min-height: 36px; }
.ticket-detail__readonly--empty { color: #777; }
.ticket-detail__explanation { white-space: pre-wrap; }
.ticket-detail__note-actions { display: flex; align-items: center; gap: 8px; margin-top: 8px; }
.ticket-detail__hint { display: block; color: #777; margin-top: 4px; }
.ticket-detail__muted { color: #777; font-size: 12px; }
...
.select, .textarea { border: 1px solid #ccc; border-radius: 4px; padding: 6px 8px; width: 100%; box-sizing: border-box; }
